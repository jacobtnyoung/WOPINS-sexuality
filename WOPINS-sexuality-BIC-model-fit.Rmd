---
title: "WOPINS Sexuality BIC Analysis"
output: 
  html_document:
    df_print: paged
    theme: lumen
    highlight: haddock
    toc: yes
    toc_float: yes
    code_fold: show
    self_contained: true
---

<style>
body {
text-align: justify}
</style>


```{r}

# set the defaults for the codechunks
knitr::opts_chunk$set( eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE )

```


```{r setup}

# ----------
# Setup 

# Clear the workspace
rm( list = ls() )

# libraries
library( here ) # for calling local directory
library( dplyr ) # for working with the data
library( network ) # for creating the network object
library( sna ) # for working with the network
library( ergm ) # for erg models
library( pander ) # for tables


# run the script to create the networks with the attributes
source( here( "WOPINS-sexuality-BUILD.R" ) )

```

This file reports the results for various BIC values for the structural terms.

```{r build-functions}

# ----
# Build the function for the separate networks
# this function takes four arguments:
# the network, a vector for the gwidegree and gwesp terms
# and a scalar of seconds to elapse before the model should terminate
eval.bic <- function( net, I, E, duration ){
  model.bics <- matrix( 0, nrow = length( I ), ncol = length( E ) )
  
  for ( i in 1:length( I ) ){
    for ( j in 1:length( E ) ){
      
      model.bics[i,j] <- 
        { setTimeLimit( duration )
          try(
          summary( ergm( net ~ edges
                         + mutual + twopath 
                         + gwidegree( decay = I[i], fixed = TRUE )
                         + gwesp( E[j], fixed = TRUE ) ) )$bic
          )
        }
    }
  }
  
  rownames( model.bics ) <- paste("I @", as.character( I ) )
  colnames( model.bics ) <- paste("E @", as.character( E ) )
  
  model.bics[grepl( "Error", model.bics ) == TRUE] <- NA
  
  model.bics <- matrix( as.numeric( model.bics ), ncol = ncol( model.bics ) )
  
  return( model.bics )
}


# ----
# build the function for the pooled networks (this includes the constraint)
eval.bic.p <- function( net, I, E, duration ){
  model.bics <- matrix( 0, nrow = length( I ), ncol = length( E ) )
  
  for ( i in 1:length( I ) ){
    for ( j in 1:length( E ) ){
      
      model.bics[i,j] <- 
        { setTimeLimit( duration )
          try( 
            summary( ergm( net ~ edges
                           + mutual + twopath 
                           + gwidegree( decay = I[i], fixed = TRUE )
                           + gwesp( E[j], fixed = TRUE ),
                           constraints = ~ blocks( "block", levels2 = c( 2, 3 ) ) ) )$bic
          )
        }
    }
  }
  
  model.bics[grepl( "Error", model.bics ) == TRUE] <- NA
  
  model.bics <- matrix( as.numeric( model.bics ), ncol = ncol( model.bics ) )
  
  rownames( model.bics ) <- paste("I @", as.character( I ) )
  colnames( model.bics ) <- paste("E @", as.character( E ) )
  
  return( model.bics )
}

```

# Friendship

## Rough approximation

Begin with some rough estimates, prior to fine tuning.

```{r, echo=TRUE}

# Set the range of values to examine
I <- seq( 0, 5, by = 1 )
E <- seq( 0, 5, by = 1 )

```

```{r, results=FALSE}

# Set the seed to reproduce results.
set.seed( 12345 )

# Estimate the models
bic.u2   <- eval.bic( u2.net, I, E, duration = 60*5 )
bic.u3   <- eval.bic( u3.net, I, E, duration = 60*5 )
bic.pool <- eval.bic.p( u2.u3.pooled.net, I, E, duration = 60*5 )

```

```{r}

print( bic.u2 )

bic.u2 == min( bic.u2, na.rm = TRUE )

```

The first table shows the BICs for the models based on the values for the structural terms.

The table shows that the best fitting value for the `gwindegree` term is 0.75 and the `gwesp` term is `r bic.u2 == min( bic.u2, na.rm = TRUE )`.

```{r}

bic.u3

bic.u3 == min( bic.u3, na.rm = TRUE )

```

The first table shows the BICs for the models based on the values for the structural terms.

The table shows that the best fitting value for the `gwindegree` term is 0.75 and the `gwesp` term is `r bic.u3 == min( bic.u3, na.rm = TRUE )`.

```{r}

bic.pool

bic.pool == min( bic.pool, na.rm = TRUE )

```

The first table shows the BICs for the models based on the values for the structural terms.

The table shows that the best fitting value for the `gwindegree` term is ??? and the `gwesp` term is `r bic.pool == min( bic.pool, na.rm = TRUE )`..

<br>


## Fine-tuning approximation

Use the information from the previous results to narrow the range of values.

```{r, echo=TRUE}

# Set the range of values to examine
I <- seq( 0, 5, by = 1 )
E <- seq( 0, 5, by = 1 )

```

```{r, results=FALSE}

# Set the seed to reproduce results.
set.seed( 12345 )

# Estimate the models
bic.u2   <- eval.bic( u2.net, seq( 0, 5, by = 1 ), seq( 0, 5, by = 1 ), duration = 60*5 )

!!!here with updating this: need to pull the values

bic.u3   <- eval.bic( u3.net, I, E, duration = 60*5 )
bic.pool <- eval.bic.p( u2.u3.pooled.net, I, E, duration = 60*5 )

```

```{r}

print( bic.u2 )

bic.u2 == min( bic.u2, na.rm = TRUE )

```

The first table shows the BICs for the models based on the values for the structural terms.

The table shows that the best fitting value for the `gwindegree` term is 0.75 and the `gwesp` term is `r bic.u2 == min( bic.u2, na.rm = TRUE )`.

```{r}

bic.u3

bic.u3 == min( bic.u3, na.rm = TRUE )

```

The first table shows the BICs for the models based on the values for the structural terms.

The table shows that the best fitting value for the `gwindegree` term is 0.75 and the `gwesp` term is `r bic.u3 == min( bic.u3, na.rm = TRUE )`.

```{r}

bic.pool

bic.pool == min( bic.pool, na.rm = TRUE )

```

The first table shows the BICs for the models based on the values for the structural terms.

The table shows that the best fitting value for the `gwindegree` term is ??? and the `gwesp` term is `r bic.pool == min( bic.pool, na.rm = TRUE )`..

<br>



----

# Power

!!!NEED TO BUILD THIS OUT ONCE YOU GET THE MODEL ABOVE TO WORK

----

<br>
<br>

###### ***Last updated `r format(Sys.time(), '%d %B, %Y')`***